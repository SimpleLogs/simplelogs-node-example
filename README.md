...
requestScope.run(correlation, () =>
  withTrace(handler, { name: touchpoint, carrier: req.headers }),
);
```
...
headers yourself. A call from curl or another server simply has none and starts
its own trace.

`withTrace()` seeds this request's spans with the browser trace that fired the
fetch, so the server work joins the page's tree instead of a detached one. Hand
it the request's headers as the `carrier`: since 2.0.0 the SDK reads the W3C
`traceparent` the browser half writes, rather than taking trace ids the caller
has picked out of the headers itself. It also isolates concurrent requests from
